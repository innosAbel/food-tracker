...
def index():
    ...
    for i in results:
        ...
        single_date['pretty_date'] = datetime.strftime(d, '%B %m, %Y')  
...
def view(date):
    ...
    query = db.execute('select id, entry_date from log_date where entry_date = ?', [date])
    date_result = query.fetchone()
    ...
    food_results = food_query.fetchall()

    log_query = db.execute('select food.name, food.protein, food.carbohydrates, food.fat, food.calories from log_date join food_date  on food_date.log_date_id = log_date.id join food on food.id = food_date.food_id where log_date.entry_date = ?', [date])
...
def food():
    ...
    query = db.execute('select * from food')
    results = query.fetchall()
